...
async def robotTest(loop):
    ...
    async with BleakClient(theRobot_bt.address, loop=loop, device=Settings["adapter"]) as client:
        ...
        async def myRobotTasks():
            ...
            '''
            await theRobot.sendCommand(Commands.REQ_RAW)    # print out ang. veloc.
            theRobot.updateMotor("left",205)       # run at a certain power for 10 s
            theRobot.updateMotor("right",205)
            for i in range(200):    # wait for ten seconds, but...
        ...
        async def motorLoop():
            ...
            while (tNow-t0) < 11:    # stop after about 11 s
                tNow = time.clock_gettime(time.CLOCK_MONOTONIC)
