...
// Global variables
float roll, pitch;      // Angles of tilt about Y and X axes, respectively
...
void loop() {
  ...
  if( myICM.dataReady() ){
    ...
    // Version 1: use arctangent. Big fluctuation near 90 degrees because aZ is about zero.
    //roll = atan2(aX,aZ)*180/M_PI;
    //pitch = atan2(aY,aZ)*180/M_PI;
    a = sqrt(pow(aX,2)+pow(aY,2)+pow(aZ,2));
    roll = asin(aY/a)*180/M_PI;
    pitch = asin(aX/a)*180/M_PI;
