...
ICM_20948_I2C IMU;  // Create an ICM_20948_I2C object. This will be used to get data from the IMU.
...
int bytestream_active = 0;    // when this is nonzero, the device starts streaming. can be 4 or 8 (# bytes)
...
float data32[3];              // array of 32-bit floating-point numbers to send (as opposed to doubles)
uint64_t data64[3];           // array of unsigned long numbers to send (could also be type double)

// For motor control
int address, direction, power;
...
//  #endif
  ...
  float thXCal = 0, thYCal = 0, thZCal = 0; // angles to measure during calib. while gyro is stationary
  tNow = micros();
...
void loop()
{
    ...
    switch (cmd->command_type)
    {
      case SET_MOTORS:
      {
        ...
        for(int i=0; i<2; i++){ // run this twice
          ...
          power = (int)cmd->data[i];    // left is 0; right is 1
          // Power value may be negative, but the motors require a positive signal.
          // Decode it into abs(power) and direction.
          if(power>0 || power==0){  // positive or zero value
  ...
  if (bytestream_active)
  {
      ...
      start=micros();
    #endif
    //res_cmd->command_type = BYTESTREAM_TX; // we are letting the command switch take care of this now
    ...
    switch(bytestream_active){
      case 4:                 //asked for a 4-byte number!
      {
        ...
        res_cmd->length=2+bytestream_active*len; // then 4 bytes for each uint32_t and 8 for each uint64_t
