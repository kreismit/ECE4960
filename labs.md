...
Entered the folder generated by extracting the ZIP. Ran `./main.py` twice while the Artemis Nano was powered on to discover the board. Manually cached the Artemis' MAC address to `settings.py` to avoid occasional connection errors.
...
Note that most of the "rate" in this case is the latency of returning pings. By measuring the time difference between ping reponses from the Artemis side, I measured the average ping latency as *160.0 ms*. The average total RTT as measured by the Python script was *159.3 ms*. These nearly identical numbers indicate that very little time is spent sending data from the Artemis.
...
Note that although the floating-point value was `2.71828`, there were additional random digits after the 8. This means that float comparison involves a finite error; comparisons must always be of the form `if( abs(float1-float2) < tolerance )` rather than `if( float1 == float2 )`.
...
The Python script indicates that it receives 14 bytes of data from the Arduino. The calculated transfer rate follows the below histogram, with mean rate **963 bytes/second**. Note this is the average over the packets which were received; only **65%** of the packets made it from the Arduino to the computer.
...
The Python script indicates that it receives 26 bytes of data from the Arduino. The calculated transfer rate follows the below histogram, with mean rate **1.46 KB/second**. Note this is the average over the packets which were received; only **22%** of the packets made it from the Arduino to the computer.

<image width="600" src="Lab2/Images/64bitTransferHistogram.png"></image>

## Lessons Learned

At first, VirtualBox saw neither the Bluetooth module in my laptop nor the USB dongle in the lab kit. However, my laptop's host OS saw both. After a quick web search, I found [this useful SuperUser post](https://superuser.com/questions/956622/no-usb-devices-available-in-virtualbox) which points out that users must be part of the `vboxusers` group on the host machine, or else no USB devices are accessible from VirtualBox. This and other issues were solved when I did the following, in order:
...
I found that the code running on the Artemis does not "care" what length is sent with the message, but the Python code does. If I specified a length different from that of the format string, I got an error. If the format string matched the length, but the Artemis sent more data, then it was cut off after `length` bytes.
