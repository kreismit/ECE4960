...
float r[2];                 // setpoints for angular velocity controller and for linear open-loop
float kp=0.75, ki=1.5, kd=0;  // P, I, and D gains for controller; can be set via Bluetooth
float e=0;                  // current error values
float eLast=0;              // previous error value
float inte=0;               // integral of error
float de;                   // change in error (differential)
float alphaE=0.8;           // lag filter parameter
float output=0;             // output of PID to send to motors

// For motor control (with linear offsets)
...
byte lFwd = 0;
byte lRev = 1;
float offsetFB = -5;        // compensate away forward driving preference (F-B)
float calib = 0.95;         // compensate away more powerful motor on one side (L/R)
float offsetLR = -5;        // compensate away more powerful motor on one side (L-R)
...
void loop()
{
  ...
  if (l_Rcvd > 1) //Check if we have a new message from amdtps_main.c through BLE_example_funcs.cpp
  {
    ...
    switch (cmd->command_type)
    {
      ...
      case SET_GAINS:
      {
        // Sets PID gains for angular velocity control...
        Serial.println("Setting control gains...");
        kp = ((float*)cmd->data)[0];
        ki = ((float*)cmd->data)[1];
        kd = ((float*)cmd->data)[2];
        // and linear offsets for open-loop straight-line driving
        offsetFB = ((float*)cmd->data)[3];
        calib = ((float*)cmd->data)[4];
        offsetLR = ((float*)cmd->data)[5];
        Serial.printf("Angular: Kp = %3.2f, Ki = %3.2f, Kd = %3.2f\n",kp,ki,kd);
        Serial.printf("Linear: offset(F-B) = %3.2f, calib (L/R) = %3.2f, offset (L-R) = %3.2f\n",offsetFB,calib,offsetLR);
        break;
      }
      case SER_RX:
      {
...
//    #endif
  ...
  */
  ...
  if( IMU.dataReady() ){
    ...
    //aCX = 10*(cos(pitchRad)*(aX-aXCal)+sin(pitchRad)*(aZ-aZCal));  // accel., calibrated and rotated
  ...
  if(pid){ // As long as we're running the controller,
    ...
    /* Remember: clockwise is direction 1. 
       ...
       However, for spinning, the robot actually spins in the same direction, from the floor's
       perspective, regardless of which way is up.                                                                             */
